"""CSC108: Fall 2024 -- Assignment 2: Bridges
...
taking the CSC108 course at the University of Toronto Mississauga. Copying for purposes
...
def find_bridges_in_radius(bridges: list[list], lat: float, lon: float,
    ...
    for bridge in bridges:
        if bridge[COLUMN_ID] not in exclusions:
            ...
            if calculate_distance(bridge_lat,
                                  bridge_lon, lat, lon) <= radius:
...
def calculate_average_condition(bridge: list, start: int, stop: int) -> float:
    ...
    0.0
    >>> calculate_average_condition(bridge, 2013, 2005)
    70.525
    """
...
def clean_span_data(raw_spans: str) -> list[float]:
    ...
    [12.0, 19.0, 21.0, 12.0]
    >>> clean_span_data('Total=64  (1)=12;(2)=19;(3)=21;(4)=42;')
    [12.0, 19.0, 21.0, 42.0]
    """
