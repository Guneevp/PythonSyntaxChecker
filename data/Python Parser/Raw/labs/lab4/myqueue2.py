...
class Queue:
    ...
    """
    # === Private Attributes ===
    # _first: The first item in the queue
    # _last: The last item in the queue
    ...
    def enqueue(self, item: Any) -> None:
        ...
        if not self.is_empty():
            self._last.next = self._last = _Node(item)
        else:
            self._first = self._last = _Node(item)
    ...
    def dequeue(self) -> Optional[Any]:
        ...
        """
        if not self.is_empty():
            out = self._first.item
            self._first = self._first.next
            return out


class _Node:
    """A private node class used in the implementation of Queue.

    === Attributes ===
    item: The stored item of the node.
    next: The next node. None if there is no next node.
    """
    item: Any
    next: Optional[_Node]
