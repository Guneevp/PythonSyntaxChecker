...
class LinkedList:
    ...
    def __init__(self, items: list) -> None:
        ...
        """
        if items:
            curr = self._first = _Node(items[0])
            for item in items[1:]:
    ...
    def __getitem__(self, index: Union[int, slice]) -> Union[Any, LinkedList]:
        ...
        if isinstance(index, int):
            ...
            curr = self._first
            step = index.step
            peter = list(range(index.start, index.stop, index.step))
            index = 0
            while curr.next is not None:
                if index in peter:
                    # curr2.next = _Node(curr.item)
                    # curr2 = curr2.next
                    shaurya.insert(len(shaurya)-1, curr.item)
                index += step
            return shaurya
...
class AidenLinkedList(LinkedList):
    def __init__(self, items: list) -> None:
