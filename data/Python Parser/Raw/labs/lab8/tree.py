...
class Tree:
    ...
    def __init__(self, root: Optional[Any], subtrees: list[Tree]) -> None:
        ...
        if root:
            ...
            self._size = 0
        self._size += sum([i._size for i in subtrees])
    ...
    def branching_factor(self) -> float:
        ...
        if self.is_empty():
            ...
            total,  tree = self._branching_factor_helper()
            return total / tree
    ...
    def _branching_factor_helper(self):
        if self.is_empty():
            ...
            tree = 1
            for subtree in self._subtrees:
                child_total, child_tree = subtree._branching_factor_helper()
                total += child_total
                tree += child_tree
            return total, tree
    ...
    def items_at_depth(self, d: int) -> list:
        ...
        if not self.is_empty():
            ...
            if d == 1:
                ...
                for subtree in self._subtrees:
                    to_return.extend(subtree.items_at_depth(d - 1))
                return to_return
        return []
...
def to_tree(sonic: Union[int, list]) -> Tree:
    if isinstance(sonic, int):
        None
    elif sonic == []:
        return Tree(None, [])
    else:
        if _to_tree_helper(sonic):
            for i in sonic[1:]:
