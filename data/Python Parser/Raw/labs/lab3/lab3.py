...
class RandomPlayer(Player):
    """ A non user controller player class that selects
    a random legal move for each of its moves.
    """

    def move(self, current, min_step, max_step, goal):
        """ Returns a random int in between the minimum legal
        move and the maximum legal move including both end
        points.
        """
        return random.randint(min_step, max_step)

class UserPlayer(Player):
    """A user controlled player class that prompts the
    user for its moves.
    """

    def move(self, current, min_step, max_step, goal):
        num = input(f"{self.name}! It is your move!\nEnter an amount between "
                        f"{min_step} and {max_step} to try to win!\n")
        while not num.isnumeric() or not (min_step <= int(num) <= max_step):
            print("Oops, looks like you entered an invalid move amount."
             "Remember that you need to input a"
              "legal number between the specified amounts!")
            num = input(f"{self.name}! It is your move!\nEnter an amount between"
                f"{min_step} and {max_step} to try to win!\n")
        return int(num)
...
class StrategicPlayer(Player):
    """Strategic Player ahh type shit

    """
    def move(self, current, min_step, max_step, goal):
        num = (goal - current) % (max_step + 1)
        if num < min_step:
            return min_step
        else: return num
