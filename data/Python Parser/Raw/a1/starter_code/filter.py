    ...
    def apply(self, customers: list[Customer],
        ...
        for customer in customers:
            if customer.get_id() == filter_string:
                is_valid = True
                numbers_to_check.extend(customer.get_phone_numbers())
        if is_valid:
            for call in data:
                if (call.src_number or call.dst_number) in numbers_to_check:
    ...
    def apply(self, customers: list[Customer],
        ...
        try:
            if filter_string[0] not in "GL":
                return data
            if len(str(int(filter_string[1:]))) > 3:
                return data
        except Exception:
            return data
        to_return = []
        # Check whether we're doing greater than or less than
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        Do not mutate any of the function arguments!
        """
        if filter_string.count(",") != 3:
            return data
        spliced_fdata = filter_string.split(",")
        try:
            # Try changing all the spliced filter data into floats
            for i in range(len(spliced_fdata)):
                spliced_fdata[i] = float(spliced_fdata[i])
        except Exception:
            return data
        # Check to see if the data is put in the right order
        if spliced_fdata[0] > spliced_fdata[2]:
            return data
        elif spliced_fdata[1] > spliced_fdata[3]:
            return data
        if spliced_fdata[0] < MAP_MIN[0] or \
            spliced_fdata[1] < MAP_MIN[1] or \
            spliced_fdata[2] < MAP_MAX[0] or \
                spliced_fdata[3] < MAP_MAX[1]:
            return data
        # Now actually check all the calls
        to_return = []
        for call in data:
            if spliced_fdata[0] <= call.src_loc[0] <= spliced_fdata[2]\
                    and spliced_fdata[1] <= call.src_loc[1] <= spliced_fdata[3]:
                to_return.append(call)
            elif spliced_fdata[0] <= call.dst_loc[0] <= spliced_fdata[2]\
                    and spliced_fdata[1] <= call.dst_loc[1] <= spliced_fdata[3]:
                to_return.append(call)
        return to_return
